...
extension Song: ExportableModel {
    struct ExternalRepresentation: Codable {
        init(exporting song: Song) {
            self.id = song.id
            self.creationDate = song.creationDate
            self.lastModified = song.lastModified
            ...
            self.trackTotal = song.trackTotal
            self.discNumber = song.discNumber
            self.discTotal = song.discTotal
            self.lyrics = song.lyrics
            self.rating = song.rating
            self.bpm = song.bpm
            self.comment = song.comment
            self.grouping = song.grouping
            self.mcn = song.mcn
            self.isrc = song.isrc
            self.musicBrainzReleaseID = song.musicBrainzReleaseID
            self.musicBrainzRecordingID = song.musicBrainzRecordingID
        }
    ...
    func export(with context: ModelExportContext) throws -> ExternalRepresentation {
        ExternalRepresentation(exporting: self)
    }
}
